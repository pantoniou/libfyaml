...
including ASAN builds on Linux and macOS, runs daily, and every platform
above runs weekly.
...
* Durable arena garbage collection needs an atomic directory exchange, which
  only Linux and macOS provide; `fy_durable_arena_gc()` fails with `ENOSYS`
  elsewhere.
* Durable arenas have a default fixed base address on Linux and macOS
  (x86-64 and arm64) and on the BSDs and illumos (x86-64); on other targets
  pass an explicit `region_base`.
