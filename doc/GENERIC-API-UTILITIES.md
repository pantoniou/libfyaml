...
// Python-style slicing with negative indices (count from end)
fy_generic fy_slice_py(fy_generic seq, ssize_t start, ssize_t end);
fy_generic fy_slice_py(struct fy_generic_builder *gb, fy_generic seq, ssize_t start, ssize_t end);
```
...
// Get middle elements (local allocation)
...
// With builder for persistent results
struct fy_generic_builder *gb = fy_generic_builder_create(NULL);
fy_generic persistent_middle = fy_slice(gb, items, 1, 4);

// Python-style negative indices (local allocation)
fy_generic last_two = fy_slice_py(items, -2, -1);  // ["d"]

// Python-style with builder
fy_generic last_three = fy_slice_py(gb, items, -3, -1);  // ["c", "d"]
```
...
fy_generic fy_first(struct fy_generic_builder *gb, fy_generic seq);

fy_generic fy_last(fy_generic seq);
fy_generic fy_last(struct fy_generic_builder *gb, fy_generic seq);
```
...
// Local (temporary) allocation
fy_generic first = fy_first(items);   // "alice"
fy_generic last = fy_last(items);     // "charlie"

// With builder for persistent results
struct fy_generic_builder *gb = fy_generic_builder_create(NULL);
fy_generic persistent_first = fy_first(gb, items);
fy_generic persistent_last = fy_last(gb, items);
...
fy_generic fy_rest(struct fy_generic_builder *gb, fy_generic seq);

fy_generic fy_tail(fy_generic seq);  // Alias
fy_generic fy_tail(struct fy_generic_builder *gb, fy_generic seq);
```
...
// With builder for persistent results
struct fy_generic_builder *gb = fy_generic_builder_create(NULL);
fy_generic persistent_rest = fy_rest(gb, items);
...
fy_generic fy_take(struct fy_generic_builder *gb, fy_generic seq, size_t n);

fy_generic fy_drop(fy_generic seq, size_t n);
fy_generic fy_drop(struct fy_generic_builder *gb, fy_generic seq, size_t n);
```
...
// With builder for persistent results
struct fy_generic_builder *gb = fy_generic_builder_create(NULL);
fy_generic persistent_first = fy_take(gb, items, 5);
fy_generic persistent_skip = fy_drop(gb, items, 3);
