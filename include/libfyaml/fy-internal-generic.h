...
static inline fy_generic fy_generic_mappingp_get_at_key(const fy_generic_mapping *mapp, size_t idx)
{
	if (!mapp || idx >= mapp->count)
		return fy_invalid;
	return mapp->pairs[idx].key;
...
static inline fy_generic fy_generic_mapping_get_at_key(fy_generic map, size_t idx)
{
