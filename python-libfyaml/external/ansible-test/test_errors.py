...
_USING_LIBFYAML = 'libfyaml' in __import__('yaml').__name__
...
@pytest.mark.parametrize("content, expected_message, expect_help_text, line, col", (
    ...
    (1, "expected string or bytes-like object, got '_AnsibleTaggedInt'" if _USING_LIBFYAML else 'a string or stream input is required', False, None, None),  # wrong type (misuse of API)

    # cases where the underling MarkedYAMLError is used

    ('k1: v1\n  k2: v2', 'Invalid multiline plain key.' if _USING_LIBFYAML else 'Mapping values are not allowed in this context.', False, 1 if _USING_LIBFYAML else 2, 5),
    ('k1: v1\n  k2: ":"', 'Invalid multiline plain key.' if _USING_LIBFYAML else 'Mapping values are not allowed in this context.', False, 1 if _USING_LIBFYAML else 2, 5),

    (':', 'Bare : detected (illegal in pyyaml mode).' if _USING_LIBFYAML else 'While parsing a block mapping did not find expected key.', False, 1, 1),
    ('!!map 1', 'Expected a mapping node, but found scalar.', False, 1, 1),
    ('[]: bad', 'While constructing a mapping found unhashable key.', False, 1, 1),
    ('{}: bad', 'While constructing a mapping found unhashable key.', False, 1, 1),
    ('"\\"', 'Double-quoted scalar without closing quote.' if _USING_LIBFYAML else 'While scanning a quoted scalar found unexpected end of stream.', False, 1, 2 if _USING_LIBFYAML else 4),
