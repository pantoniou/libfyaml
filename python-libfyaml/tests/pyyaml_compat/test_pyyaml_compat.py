...
""", Loader=TestLoader)
    ...
    def test_anchor_access_methods(self):
        ...
        # Anchors are stripped by default when resolving (visual clutter in
        # resolved docs); keep_anchors=True preserves them as metadata.
        doc = fy.loads("value: &myanchor test", keep_anchors=True)
        assert doc['value'].has_anchor()
        assert doc['value'].get_anchor() == 'myanchor'

        # Default: anchors are stripped on resolve.
        doc_default = fy.loads("value: &myanchor test")
        assert not doc_default['value'].has_anchor()

        doc2 = fy.loads("value: plain", keep_anchors=True)
