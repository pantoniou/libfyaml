...
class TestNodeWrapperMarks:
    """Test that _NodeWrapper uses markers for start_mark/end_mark."""

    def test_node_wrapper_without_markers(self):
        """Without markers, start_mark/end_mark should be None."""
        from libfyaml.pyyaml_compat import _NodeWrapper
        doc = fy.loads('foo: bar')
        wrapper = _NodeWrapper(doc['foo'])
        assert wrapper.start_mark is None
        assert wrapper.end_mark is None

    def test_node_wrapper_with_markers(self):
        """With markers, start_mark/end_mark should be Mark objects."""
        from libfyaml.pyyaml_compat import _NodeWrapper
        from libfyaml.pyyaml_compat.error import Mark
        doc = fy.loads('foo: bar\n', create_markers=True)
        wrapper = _NodeWrapper(doc['foo'])
        assert wrapper.start_mark is not None
        assert wrapper.end_mark is not None
        assert isinstance(wrapper.start_mark, Mark)
        assert isinstance(wrapper.end_mark, Mark)
        # Check mark attributes
        assert wrapper.start_mark.line == 0
        assert wrapper.start_mark.column == 5
        assert wrapper.end_mark.line == 0
        assert wrapper.end_mark.column == 8
