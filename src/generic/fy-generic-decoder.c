...
static fy_generic fy_generic_decoder_create_scalar(struct fy_generic_decoder *gd, struct fy_event *fye,
		fy_generic va, fy_generic vt, fy_generic vcomment, fy_generic vmarker);
...
fy_generic_compose_process_event(struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path, void *userdata)
{
	...
	fy_generic v, va, vt, vcomment, vmarker, vds;
	...
	if (gd->keep_comments && (gd->parse_flags & FYGDPF_KEEP_COMMENTS) &&
		fy_token_has_any_comment(fyt)) {
	...
	if ((gd->parse_flags & FYGDPF_CREATE_MARKERS) && fyt) {

		start_mark = fy_token_start_mark(fyt);
		end_mark = fy_token_end_mark(fyt);

		/* start(input pos, line, column) - end(input pos, line, column) */
	...
	case FYET_SCALAR:
		...
		v = fy_generic_decoder_create_scalar(gd, fye, va, vt, vcomment, vmarker);
		fyp_error_check(fyp, fy_generic_is_valid(v), err_out,
...
fy_generic_decoder_create(struct fy_parser *fyp, struct fy_generic_builder *gb)
{
	...
	gd->keep_comments = !!(fyp->cfg.flags & FYPCF_PARSE_COMMENTS);
	gd->keep_markers = false;
	gd->vroot = fy_invalid;
	gd->vds = fy_invalid;
