...
struct fy_event *fy_encode_generic_string_event(struct fy_generic_encoder *fyge, fy_generic v)
{
	...
	fye = fy_emit_event_create(fyge->emit, FYET_SCALAR, FYSS_ANY, szstr.data, szstr.size, gd->anchor, gd->tag);
	(void)fy_encode_generic_attach_comments(fyge, gd, fye);
	return fye;
}
...
int fy_encode_generic_sequence(struct fy_generic_encoder *fyge, fy_generic v)
{
	...
	fye = fy_emit_event_create(fyge->emit, FYET_SEQUENCE_START, FYNS_ANY, gd->anchor, gd->tag);
	if (!fye)
		goto err_out;
...
int fy_encode_generic_mapping(struct fy_generic_encoder *fyge, fy_generic v)
{
	...
	fye = fy_emit_event_create(fyge->emit, FYET_MAPPING_START, FYNS_ANY, gd->anchor, gd->tag);
	if (!fye)
		goto err_out;
