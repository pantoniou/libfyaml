/*
 * fy-reflection-util.c - reflection utils
 *
...
#define STRTOXF_IS_UNSIGNED		FY_BIT(0)
#define STRTOXF_IS_SIGNED		FY_BIT(1)
#define STRTOXF_SKIP_UNDERSCORE		FY_BIT(2)
#define STRTOXF_ALLOW_BASE2		FY_BIT(3)
#define STRTOXF_ALLOW_BASE8		FY_BIT(4)
#define STRTOXF_ALLOW_BASE16		FY_BIT(5)
#define STRTOXF_SINGLE_ZERO		FY_BIT(6)
...
{ \
	...
	while ((c = *str++) != '\0') { \
		...
		if (dv < 0 || dv >= (int)base) \
			return -EINVAL; \
		if (FY_MUL_OVERFLOW(v, (_type)base, &v) || \
		    FY_ADD_OVERFLOW(v, (_type)(!negative ? dv : -dv), &v)) \
			return -ERANGE; \
	} \
	*res = v; \
	return 0; \
...
/* logging, common infra because it's quite complex */
	   ...
	   const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	fy_rfl_vlog(ctx, error_type, fmt, ap);
	va_end(ap);
}
...
void reflection_walker_print_path(struct reflection_walker *rw)
{
	...
	for (i = 0; i < count; i++) {
		...
		fprintf(stderr, "  RWT %s(%s)%s%s%s%s%s%s%s%s%s%s",
		...
		if (rwt->flags & RWF_SEQ_IDX) {
			rtd = rwt->parent->rtd;
			fprintf(stderr, " seq-idx=%ju (%s[%ju])",
		...
		if (rwt->flags & RWF_FIELD_IDX) {
			...
			if (rtd->ti->kind == FYTK_STRUCT || rtd->ti->kind == FYTK_UNION) {
				...
				fprintf(stderr, " field-idx=%ju (%s.%s)",
						rwt->idx, rtd->ti->name, rfd->fi->name);
			} else {
				fprintf(stderr, " field-idx=%ju NON-STRUCT (%s)",
						rwt->idx, rtd->ti->name);
			}
		}
