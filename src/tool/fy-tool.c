...
struct reflection_type_ops {
	...
	int (*setup_child)(struct reflection_object *ro, struct reflection_object *ro_parent, struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path);
...
struct struct_instance_data {
	...
	struct reflection_field_data *rfd_key;
...
#define REFLECTION_OBJECT_SKIP	1
...
int
reflection_object_setup_child(struct reflection_object *ro, struct reflection_object *ro_parent,
			      struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path);
...
int
const_array_setup_child(struct reflection_object *ro, struct reflection_object *ro_parent,
			struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	struct const_array_instance_data *id;
	struct reflection_type_data *rtd_dep;
	...
	ret = reflection_object_setup(ro, ro_parent, (void *)(uintptr_t)idx,
...
static void struct_cleanup(struct reflection_object *ro)
{
	...
	id = &ro->id_struct;
	if (id->ro_flatten) {
		reflection_object_cleanup(id->ro_flatten);
		free(id->ro_flatten);
	}
...
static int struct_setup(struct reflection_object *ro, struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	...
	if (ro->rtd->key_field) {
		id->rfd_key = reflection_type_data_lookup_field(ro->rtd, ro->rtd->key_field);
		if (!id->rfd_key)
			goto err_out;
	} else
		id->rfd_key = NULL;
	...
	if (id->rfd_flatten) {
		...
		id->ro_flatten = malloc(sizeof(*id->ro_flatten));
		if (!id->ro_flatten)
			goto err_out;

		rc = reflection_object_setup(id->ro_flatten, ro,
					     id->rfd_flatten, id->rfd_flatten->rtd,
					     fyp, fye, path, field_data,
					     id->rfd_flatten->fi->type_info->size);
		if (rc)
			goto err_out;
	}
...
int
struct_setup_child(struct reflection_object *ro, struct reflection_object *ro_parent,
		   struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	struct reflection_type_data *rtd;
	struct reflection_field_data *rfd;
	...
	void *field_data;
	int field_idx, rc;
	bool ro_instantiated = false;
	struct fy_path_component *fyc;
	struct fy_document *fyd_complex_key;

	id = &ro_parent->id_struct;
	if (id->ro_flatten)
		return reflection_object_setup_child(ro, id->ro_flatten, fyp, fye, path);
	...
	fyc = fy_path_last_not_collection_root_component(path);
	fyt_key = fy_path_component_mapping_get_scalar_key(fyc);
	if (!fyt_key) {
		/* if we're allowing skipping... */
		if (rtd->skip_unknown)
			return REFLECTION_OBJECT_SKIP;

		fyd_complex_key = fy_path_component_mapping_get_complex_key(fyc);
		if (!fyd_complex_key) {
			fy_event_report(fyp, fye, FYEP_VALUE, FYET_ERROR,
				"no scalar or complex key in struct '%s'", rtd->ti->name);
			goto err_out;
		}

		/* complex keys not supported */
		fy_event_report(fyp, fye, FYEP_VALUE, FYET_ERROR,
			"complex key in struct '%s' is not supported", rtd->ti->name);
		goto err_out;
	}
	...
	rc = reflection_object_setup(ro, ro_parent, rfd, rfd->rtd,
				     fyp, fye, path, field_data, ti->size);
	if (rc)
		goto err_out;

	ro_instantiated  = true;
...
static int ptr_setup(struct reflection_object *ro, struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	...
	*(void **)data = p;
	...
	rc = ro->rtd->ops->setup(ro, fyp, fye, path);
	if (rc)
		goto err_out;
...
int ptr_char_copy(struct reflection_type_data *rtd, void *dst, const void *src)
{
	const char *src_str;
	size_t len;
	char *p;
	...
	assert(*(char **)dst == NULL);

	src_str = *(void **)src;

	len = strlen(src_str);
	p = reflection_malloc(rtd->rts, len + 1);
	if (!p)
		goto err_out;
	memcpy(p, src_str, len);
	p[len] = '\0';
...
{
	struct dyn_array_instance_data *id;
	struct reflection_type_data *rtd_final;
	...
	if (ro->rtd->counter) {
		...
		if (!id->rfd_counter) {
			fy_event_report(fyp, fye, FYEP_VALUE, FYET_ERROR,
					"counter field %s.%s not found", ro->rtd->ti->fullname, ro->rtd->counter);
			goto err_out;
		}
	} else {
		id->rfd_counter = NULL;
	...
	rtd_final = reflection_type_data_final_dependent(ro->rtd);
	assert(rtd_final);

	if (!rtd_final->key_field && fye->type != FYET_SEQUENCE_START) {
		fy_event_report(fyp, fye, FYEP_VALUE, FYET_ERROR,
				"Illegal event type (expecting sequence start)");
		goto err_out;
	}

	if (rtd_final->key_field && fye->type != FYET_MAPPING_START) {
		fy_event_report(fyp, fye, FYEP_VALUE, FYET_ERROR,
				"Illegal event type (expecting mapping start)");
		goto err_out;
	}

	if (rtd_final->key_field)
		id->key_field = rtd_final->key_field;
...
int
dyn_array_setup_child(struct reflection_object *ro, struct reflection_object *ro_parent,
		      struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	struct dyn_array_instance_data *id;
	struct reflection_type_data *rtd_dep;
	...
	rc = reflection_object_setup(ro, ro_parent, (void *)(uintptr_t)idx,
				     rtd_dep, fyp, fye, path, data, item_size);
	if (rc)
		goto err_out;
...
static const struct reflection_type_ops dyn_array_ops = {
	...
	.setup_child = dyn_array_setup_child,
	.emit = dyn_array_emit,
	.dtor = dyn_array_dtor,
};
...
static const struct reflection_type_ops reflection_ops_table[FYTK_COUNT] = {
	...
	[FYTK_STRUCT] = {
		...
		.setup_child = struct_setup_child,
		.emit = struct_emit,
		.dtor = struct_dtor,
	},
	[FYTK_UNION] = {
	...
	[FYTK_CONSTARRAY] = {
		...
		.setup_child = const_array_setup_child,
...
reflection_object_setup_child(struct reflection_object *ro, struct reflection_object *ro_parent,
			      struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path)
{
	if (!ro || !ro_parent || !fye || !path || !ro_parent->rtd->ops->setup_child)
		return -1;

	return ro_parent->rtd->ops->setup_child(ro, ro_parent, fyp, fye, path);
}
...
void reflection_type_data_destroy(struct reflection_type_data *rtd)
{
	...
	// fprintf(stderr, "%s: #%d %s actual destroy\n", __func__, rtd->idx, rtd->ti->fullname);

	rts = rtd->rts;
	assert(rts);

	if (rtd->default_value) {
		reflection_type_data_call_dtor(rtd, rtd->default_value);
		reflection_free(rts, rtd->default_value);
	}

	if (rtd->fill_value) {
		reflection_type_data_call_dtor(rtd->rtd_dep, rtd->fill_value);
		reflection_free(rts, rtd->fill_value);
	}

	if (rtd->terminator_value) {
		reflection_type_data_call_dtor(rtd->rtd_dep, rtd->terminator_value);
	...
	if (rtd->rtd_source)
		reflection_type_data_destroy(rtd->rtd_source);

	/* remove from the typesystem, if hooked there */
	if (rts && rtd->idx >= 0)
		rts->rtds[rtd->idx] = NULL;
...
void reflection_type_system_dump(struct reflection_type_system *rts)
{
	...
	printf("reflection_type_system_dump: root=#%d:'%s'\n",
			rts->rtd_root ? rts->rtd_root->idx : -1,
			rts->rtd_root ? rts->rtd_root->ti->fullname : "N/A");
	for (i = 0; i < rts->rtd_count; i++) {
		...
		if (rtd->rtd_source)
			printf(" src: #%d:'%s'", rtd->rtd_source->idx, rtd->rtd_source->ti->fullname);
		if (rtd->yaml_annotation_str)
...
void reflection_type_system_destroy(struct reflection_type_system *rts)
{
	...
	/* destroy recursively */
	reflection_type_data_destroy(rts->rtd_root);
	rts->rtd_root = NULL;

	/* nothing should remain */
	if (rts->rtds) {
		// fprintf(stderr, "%s: remaining garbage\n", __func__);
...
int
reflection_type_data_copy_annotations(struct reflection_type_data *rtd, struct reflection_type_data *rtd_source)
{
	struct reflection_type_system *rts;
	...
	if (rtd_source->default_value) {
		rtd->default_value = reflection_malloc(rts, rtd->ti->size);
		if (!rtd->default_value)
			goto err_out;
		memcpy(rtd->default_value, rtd_source->default_value, rtd->ti->size);
	}

	rtd->fyn_fill = rtd_source->fyn_fill;
	if (rtd_source->fill_value) {
		rtd->fill_value = reflection_malloc(rts, rtd->rtd_dep->ti->size);
		if (!rtd->fill_value)
			goto err_out;
		memcpy(rtd->fill_value, rtd_source->fill_value, rtd->rtd_dep->ti->size);
	}

	rtd->fyn_terminator = rtd_source->fyn_terminator;
	if (rtd_source->terminator_value) {
		rtd->terminator_value = reflection_malloc(rts, rtd->rtd_dep->ti->size);
		if (!rtd->terminator_value)
			goto err_out;
...
reflection_type_data_mutate(struct reflection_type_data *rtd_source, const struct reflection_type_mutation *rtm)
{
	...
	// fprintf(stderr, "%s: #%d %p (%s)\n", __func__, rtd_source->idx, rtd_source, rtm->mutation_name);
	...
	for (i = 0; i < rts->rtd_count; i++) {
		...
		if ((!rtm->ops || rtm->ops == rtd->ops) &&
		    ...
		    (!rtm->key_field || !strcmp(rtm->key_field, rtd->key_field)) &&
	...
	rtd->key_field = rtm->key_field ? rtm->key_field : rtd_source->key_field;
	...
	if (rtm->key_field)
		rtd->key_field = rtm->key_field;

	rtd->flags |= RTDF_MUTATED;
	rtd->rtd_source = reflection_type_data_ref(rtd_source);
	rtd->mutation_name = rtm->mutation_name;

	/* and copy annotations (and values) */
	rc = reflection_type_data_copy_annotations(rtd, rtd_source);
	if (rc)
		goto err_out;
				 ...
				 struct reflection_type_data *rtd_parent, void *parent_addr)
{
	...
	case FYTK_STRUCT:
		...
		key_field = fy_type_info_get_yaml_string(rtd->ti, "key");
		if (key_field) {
			assert(!rtd->key_field);

			// fprintf(stderr, ">>>> struct %s key-field=%s\n", rtd->ti->name, key_field);
			...
			reflection_type_mutation_reset(&rtm);
			rtm.mutation_name = "key";
			rtm.key_field = key_field;
...
struct reflection_object *
reflection_decoder_object_alloc(struct reflection_decoder *rd)
{
	struct reflection_object *ro;
...
void
reflection_decoder_object_recycle(struct reflection_decoder *rd, struct reflection_object *ro)
{
	ro->next = rd->ro_free;
	rd->ro_free = ro;
}
...
reflection_compose_process_event(struct fy_parser *fyp, struct fy_event *fye, struct fy_path *path, void *userdata)
{
	...
	struct reflection_object *ro, *ro_parent, ro_scalar;
	...
	if (rd->ro_consumer && fye->type != FYET_NONE) {
		...
		if (rc < 0)
			return FYCR_ERROR;
		if (rc == 1) {
			rc = reflection_object_finish(rd->ro_consumer, fyp, fye, path);
	...
	case FYET_MAPPING_START:
		...
		ro = fye->type != FYET_SCALAR ? reflection_decoder_object_alloc(rd) : &ro_scalar;
		if (!ro)
			return FYCR_ERROR;

		ro_parent = fy_path_get_parent_user_data(path);
		rc = !ro_parent ?
			reflection_object_setup(ro, NULL, NULL, rd->entry, fyp, fye, path, rd->data, rd->data_size) :
			reflection_object_setup_child(ro, ro_parent, fyp, fye, path);

		/* handle skip */
		if (rc == REFLECTION_OBJECT_SKIP) {
			rd->skip_start = fy_path_last_component(path);
			if (fye->type != FYET_SCALAR)
				reflection_decoder_object_recycle(rd, ro);
			return FYCR_OK_CONTINUE;
		}
...
reflection_parse(struct fy_parser *fyp, struct reflection_type_data *rtd)
{
	...
	data = reflection_malloc(rts, rtd->ti->size);
	if (!data)
		goto err_out;
