...
#define OPT_CFLAGS			2024
#define OPT_GENERATE_C			2025
#define OPT_IMPORT_BLOB			2026
#define OPT_GENERATE_BLOB		2027
#define OPT_NO_PRUNE_SYSTEM		2028
#define OPT_TYPE_INCLUDE		2029
#define OPT_TYPE_EXCLUDE		2030
#define OPT_IMPORT_C_FILE		2031
#define OPT_ENTRY_TYPE			2032
#define OPT_ENTRY_META			2033
#define OPT_PACKED_ROUNDTRIP		2034
#define OPT_DRY_RUN			2035
#define OPT_DUMP_REFLECTION		2036
#define OPT_DEBUG_REFLECTION		2037
...
reflection_meta_dump(struct reflection_meta *rm)
{
	...
	for (id = rmvid_first_valid; id <= rmvid_last_valid; id++) {
		...
		fprintf(stderr, "%s: %c %s\n", name,
				...
				reflection_meta_value_str(rm, id));
	}
}
		  ...
		  struct reflection_walker *rw, enum reflection_emit_flags flags)
{
	...
	switch (type_kind) {
	case FYTK_FLOAT:
		rc = fy_emit_scalar_printf(emit, FYSS_PLAIN, NULL, NULL, "%.*g", FLT_DECIMAL_DIG, num.f);
		RE_ASSERT(!rc);
		break;
	case FYTK_DOUBLE:
		rc = fy_emit_scalar_printf(emit, FYSS_PLAIN, NULL, NULL, "%.*lg", DBL_DECIMAL_DIG, num.d);
		RE_ASSERT(!rc);
		break;
	case FYTK_LONGDOUBLE:
		rc = fy_emit_scalar_printf(emit, FYSS_PLAIN, NULL, NULL, "%.*Lg", LDBL_DECIMAL_DIG, num.ld);
		RE_ASSERT(!rc);
		break;
	default:
		abort();
