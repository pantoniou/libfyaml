 ...
 *
 * The bases sit above 0x700000000000: the ASAN heap uses 0x500000000000
 * (clang) and 0x600000000000 (gcc) on x86-64, 4 TiB each, so a base in that
 * range is taken in ASAN builds.
 */
#define TEST_REGION_BASE	0x700000000000ULL
...
#define TEST_INDEX_REGION_BASE	0x740000000000ULL
...
START_TEST(durable_base_conflict)
{
	char dir[256];
	struct fy_allocator *da;
	uint64_t base = 0x780000000000ULL;	/* separate from the others */
	void *occupy;
...
START_TEST(verify_checkpoint_basic)
{
	...
	/* write some content */
	for (i = 0; i < 100; i++) {
		p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "hello", 6, 8);
		ck_assert_ptr_ne(p, NULL);
	}
...
START_TEST(verify_detects_corruption)
{
	...
	p = (char *)fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "original", 9, 8);
	ck_assert_ptr_ne(p, NULL);
...
START_TEST(verify_slot_rotation)
{
	...
	for (i = 0; i < 200; i++) {
		const char *p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "data", 5, 8);
		ck_assert_ptr_ne(p, NULL);
	}
...
START_TEST(verify_multi_chunk)
{
	...
	for (fill = 0; fill + sizeof(pad) < chunk_cap; fill += sizeof(pad)) {
		const char *p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT,
							   pad, sizeof(pad), 8);
		ck_assert_ptr_ne(p, NULL);
	}
	/* a couple more to trigger the grow */
	ck_assert_ptr_ne(fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, pad, sizeof(pad), 8), NULL);
	ck_assert_ptr_ne(fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, pad, sizeof(pad), 8), NULL);
...
START_TEST(verify_cross_session)
{
	...
	p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "persist-me", 11, 8);
	ck_assert_ptr_ne(p, NULL);
...
START_TEST(verify_checkpoint_iterate)
{
	...
	for (i = 0; i < 5; i++) {
		p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "iter", 5, 8);
		ck_assert_ptr_ne(p, NULL);
		ck_assert_int_eq(fy_allocator_checkpoint(da), 0);
	}
...
START_TEST(verify_checkpoint_recover)
{
	...
	for (i = 0; i < 50; i++) {
		p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "before", 7, 8);
		ck_assert_ptr_ne(p, NULL);
	}
	...
	chunk_cap = TEST_CHUNK_SIZE;
	for (fill = 0; fill + sizeof(pad) < chunk_cap; fill += sizeof(pad)) {
		p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT,
					       pad, sizeof(pad), 8);
		ck_assert_ptr_ne(p, NULL);
	}
	/* a couple more to ensure we cross into chunk 1 */
	ck_assert_ptr_ne(fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT,
	...
	p = fy_allocator_store(a, FY_ALLOC_TAG_DEFAULT, "post-recovery", 14, 8);
	ck_assert_ptr_ne(p, NULL);
