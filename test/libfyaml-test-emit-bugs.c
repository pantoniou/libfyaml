...
START_TEST(emit_bug_newline_only_folded)
{
	ASSERT_MAPPING_VALUE_RT("\n", FY_NT, FYSS_FOLDED);
}
END_TEST
...
void libfyaml_case_emit_bugs(struct fy_check_suite *cs)
{
	...
	// XXX fy_check_testcase_add_test(ctc, emit_bug_literal_empty_in_sequence_structure);
	(void)emit_bug_literal_empty_in_sequence_structure;	// known failure
	...
	// XXX fy_check_testcase_add_test(ctc, emit_bug_newline_only_folded);
	(void)emit_bug_newline_only_folded;			// known failure
