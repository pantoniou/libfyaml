...
TCase *libfyaml_case_generic_scalars(void)
{
	TCase *tc;

	tc = tcase_create("generic-scalars");

	/* booleans */
	tcase_add_test(tc, scalar_booleans);

	/* nulls */
	tcase_add_test(tc, scalar_nulls);

	/* integers */
	tcase_add_test(tc, scalar_int_decimal);
	tcase_add_test(tc, scalar_int_octal);
	tcase_add_test(tc, scalar_int_octal_0o);
	tcase_add_test(tc, scalar_int_hex);
	tcase_add_test(tc, scalar_int_binary);
	tcase_add_test(tc, scalar_int_underscore);
	tcase_add_test(tc, scalar_int_sexagesimal);

	/* floats */
	tcase_add_test(tc, scalar_float_basic);
	tcase_add_test(tc, scalar_float_dot_prefix);
	tcase_add_test(tc, scalar_float_trailing_dot);
	tcase_add_test(tc, scalar_float_scientific);
	tcase_add_test(tc, scalar_float_underscore);
	tcase_add_test(tc, scalar_float_sexagesimal);
	tcase_add_test(tc, scalar_float_infinity);
	tcase_add_test(tc, scalar_float_nan);

	/* strings */
	tcase_add_test(tc, scalar_string_plain);

	/* edge cases */
	tcase_add_test(tc, scalar_edge_cases);

	/* special values */
	tcase_add_test(tc, scalar_special);

	/* JSON-specific behavior */
	tcase_add_test(tc, scalar_json_specific);
