...
START_TEST(unified_ops)
{
	...
	v = fy_contains(gb, vexp, 2);
	ok = v.v == fy_true_value;
	printf("> fy_contains(gb, [1,2,3], 2): ");
	fy_generic_emit_default(v);
	ck_assert(ok == true);

	v = fy_contains(vexp, 2);
	ok = v.v == fy_true_value;
	printf("> fy_contains([1,2,3], 2): ");
	fy_generic_emit_default(v);
	ck_assert(ok == true);

	v = fy_contains(gb, vexp, 99);
	ok = v.v == fy_true_value;
	printf("> fy_contains(gb, [1,2,3], 99): ");
	fy_generic_emit_default(v);
	ck_assert(ok == false);

	v = fy_contains(vexp, 99);
	ok = v.v == fy_true_value;
	...
	/* sum a sequence of numbers */
	seq = fy_sequence(7, 6, 5, 8);
	v = fy_reduce(gb, seq, 0, fy_generic_test_sum_reducer);
	sum = fy_cast(v, 0);
	ck_assert(sum == 26);
	printf("> fy_reduce(gb, [7,6,5,8]) - sum): %d", sum);
	fy_generic_emit_default(v);

	v = fy_reduce(seq, 0, fy_generic_test_sum_reducer);
	sum = fy_cast(v, 0);
	ck_assert(sum == 26);
...
START_TEST(lambda_ops)
{
	...
	ck_assert(fy_len(v) == 1);
	ck_assert(fy_get(v, 0, -1) == 101);
	printf("> fy_filter_lambda(gb, ...) (> 100): ");
	fy_generic_emit_default(v);
...
START_TEST(set_ops)
{
	...
	/* testing set, simple seq */
	seq = fy_sequence(10, 20, 30);
	v = fy_generic_op(gb, FYGBOPF_SET, seq, 2, (fy_generic []){ fy_value(1), fy_value(200)} );
	ck_assert(fy_generic_is_sequence(v));
	ck_assert(fy_len(v) == 3);
	...
	printf("> simple seq-set: ");
	fy_generic_emit_default(v);

	/* testing set, simple map */
	map = fy_mapping("foo", 10, "bar", 20);
	v = fy_generic_op(gb, FYGBOPF_SET, map, 2, (fy_generic []){ fy_value("bar"), fy_value(200)} );
	ck_assert(fy_generic_is_mapping(v));
	ck_assert(fy_len(v) == 2);
	ck_assert(fy_get(v, "foo", -1) == 10);
	ck_assert(fy_get(v, "bar", -1) == 200);
	...
	v = fy_generic_op(gb, FYGBOPF_SET, seq, 6, (fy_generic []){
			fy_value(0), fy_value(100),
			fy_value(1), fy_value(200),
			fy_value(2), fy_value(300) } );
	ck_assert(fy_generic_is_sequence(v));
	ck_assert(fy_len(v) == 3);
	ck_assert(fy_get(v, 0, -1) == 100);
	ck_assert(fy_get(v, 1, -1) == 200);
	...
	seq = fy_sequence(10, 20, 30);
	v = fy_generic_op(gb, FYGBOPF_SET_AT_PATH, seq,
			2, (fy_generic []){ fy_value(1), fy_value(200)});	// /1
	ck_assert(fy_generic_is_sequence(v));
	ck_assert(fy_len(v) == 3);
	ck_assert(fy_get(v, 0, -1) == 10);
	ck_assert(fy_get(v, 1, -1) == 200);
	ck_assert(fy_get(v, 2, -1) == 30);
	printf("> simple seq-set-at-path /1=200: ");
	fy_generic_emit_default(v);

	/* testing set_at_path, nested */
	seq = fy_sequence(10, 20, fy_sequence(1000, 2000));
	v = fy_generic_op(gb, FYGBOPF_SET_AT_PATH, seq,
			3, (fy_generic []){ fy_value(2), fy_value(0), fy_value(1)});	// /2/1
	ck_assert(fy_generic_is_sequence(v));
	ck_assert(fy_len(v) == 3);
	ck_assert(fy_get(v, 0, -1) == 10);
	...
	ck_assert(fy_get(v, "foo", -1) == 10);
	ck_assert(fy_get(v, "bar", -1) == 100);
	printf("> simple map-set-at-path /bar=100: ");
	fy_generic_emit_default(v);
	...
	/* testing set_at_path, simple polymorphic */
	seq = fy_sequence(10, 20, 30);
	v = fy_set_at_path(seq, 1, 200);	// /1
	ck_assert(fy_generic_is_sequence(v));
	ck_assert(fy_len(v) == 3);
	ck_assert(fy_get(v, 0, -1) == 10);
	ck_assert(fy_get(v, 1, -1) == 200);
	ck_assert(fy_get(v, 2, -1) == 30);
	printf("> simple seq-set-at-path (poly) /1=200: ");
	fy_generic_emit_default(v);

	/* testing set_at_path, nested, polymorphic */
	map = fy_mapping("foo", 10, "bar", fy_mapping("baz", 100));
	v = fy_set_at_path(gb, map, "bar", "baz", 1);		// /bar/baz = 1
	ck_assert(fy_generic_is_mapping(v));
	ck_assert(fy_len(v) == 2);
	ck_assert(fy_get(v, "foo", -1) == 10);
	v2 = fy_get(v, "bar", fy_invalid);
	ck_assert(fy_generic_is_mapping(v2));
	ck_assert(fy_get(v2, "baz", -1) == 1);
	printf("> nested map-set-at-path (poly) /bar/baz=1: ");
	fy_generic_emit_default(v);
...
START_TEST(parse_emit_ops)
{
	...
	args.parse.flags = FYOPPF_DISABLE_DIRECTORY | FYOPPF_INPUT_TYPE_STRING | FYOPPF_MODE_AUTO;
	...
	memset(&args, 0, sizeof(args));
	args.parse.flags = FYOPPF_DISABLE_DIRECTORY | FYOPPF_INPUT_TYPE_STRING | FYOPPF_MODE_JSON;
	...
	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_BLOCK;
	...
	map = fy_mapping("name", "Alice", "age", 30);
	fy_generic_emit_default(map);

	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_JSON | FYOPEF_STYLE_DEFAULT;
	...
	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_FLOW;
	...
	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_JSON | FYOPEF_STYLE_DEFAULT | FYOPEF_INDENT_2;
	...
	memset(&args, 0, sizeof(args));
	args.parse.flags = FYOPPF_DISABLE_DIRECTORY | FYOPPF_INPUT_TYPE_STRING | FYOPPF_MODE_YAML_1_2;
	...
	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_JSON | FYOPEF_STYLE_BLOCK;
	...
	/* test one liners without a newline */
	map = fy_mapping("name", "Alice", "age", 30);
	fy_generic_emit_default(map);

	memset(&args, 0, sizeof(args));
	args.emit.flags = FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_ONELINE;
	emitted = fy_generic_op_args(gb, FYGBOPF_EMIT, map, &args);
	ck_assert(fy_generic_is_valid(emitted));
	ck_assert(fy_generic_is_string(emitted));
	result_str = fy_generic_cast_sized_string_default(emitted, fy_szstr_empty).data;
	ck_assert_ptr_ne(result_str, NULL);
	printf("> EMIT mapping to JSON (oneline):\n%s\n", result_str);
	fy_generic_emit_default(parsed);

	/* Test PARSE operation with YAML and fy_gb_parse */
	yaml_str = fy_gb_string_create(gb, "- item1\n- item2\n- item3");
	ck_assert(fy_generic_is_valid(yaml_str));

	parsed = fy_gb_parse(gb, "- item1\n- item2\n- item3", FYOPPF_DISABLE_DIRECTORY | FYOPPF_INPUT_TYPE_STRING, NULL);
	ck_assert(fy_generic_is_valid(parsed));
	ck_assert(fy_generic_is_sequence(parsed));
	ck_assert(fy_len(parsed) == 3);
	ck_assert_str_eq(fy_get(parsed, 0, ""), "item1");
	...
	seq = fy_sequence(1, 2, 3, 4, 5);
	emitted = fy_gb_emit(gb, seq, FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_BLOCK, NULL);
	ck_assert(fy_generic_is_valid(emitted));
	ck_assert(fy_generic_is_string(emitted));
	result_str = fy_generic_cast_sized_string_default(emitted, fy_szstr_empty).data;
	ck_assert_ptr_ne(result_str, NULL);
	printf("> EMIT sequence to YAML (with fy_gb_emit):\n%s\n", result_str);

	seq = fy_sequence(1, 2, 3, 4, 5);
	emitted = fy_local_emit(seq, FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_BLOCK, NULL);
	ck_assert(fy_generic_is_valid(emitted));
	ck_assert(fy_generic_is_string(emitted));
	result_str = fy_generic_cast_sized_string_default(emitted, fy_szstr_empty).data;
	ck_assert_ptr_ne(result_str, NULL);
	printf("> EMIT sequence to YAML (with fy_local_emit):\n%s\n", result_str);

	seq = fy_sequence(1, 2, 3, 4, 5);
	emitted = fy_emit(seq, FYOPEF_DISABLE_DIRECTORY | FYOPEF_MODE_YAML_1_2 | FYOPEF_STYLE_BLOCK, NULL);
	ck_assert(fy_generic_is_valid(emitted));
	ck_assert(fy_generic_is_string(emitted));
	result_str = fy_generic_cast_sized_string_default(emitted, fy_szstr_empty).data;
	ck_assert_ptr_ne(result_str, NULL);
	printf("> EMIT sequence to YAML (with fy_emit):\n%s\n", result_str);

#if 0
	seq = fy_parse_file(FYOPPF_DISABLE_DIRECTORY, "x.yaml");
	fy_generic_emit_default(parsed);
	emitted = fy_emit_file(seq, FYOPEF_DISABLE_DIRECTORY, "y.yaml");
	fy_generic_emit_default(emitted);
#endif
...
START_TEST(conversion_to_int)
{
	...
	/* INT → INT (identity) */
	v = fy_to_generic(42);
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == 42);
	printf("> INT 42 → INT 42: ");
	fy_generic_emit_default(result);
	...
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == 1);
	printf("> BOOL true → INT 1: ");
	fy_generic_emit_default(result);

	v = fy_to_generic((_Bool)false);
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, -1) == 0);
	printf("> BOOL false → INT 0: ");
	fy_generic_emit_default(result);
	...
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == 3);
	printf("> FLOAT 3.7 → INT 3: ");
	fy_generic_emit_default(result);

	v = fy_to_generic(-2.9);
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == -2);
	printf("> FLOAT -2.9 → INT -2: ");
	fy_generic_emit_default(result);

	/* STRING → INT (parse base-10) */
	v = fy_gb_string_create(gb, "12345");
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == 12345);
	printf("> STRING \"12345\" → INT 12345: ");
	fy_generic_emit_default(result);

	v = fy_gb_string_create(gb, "-9876");
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_int(result));
	ck_assert(fy_cast(result, 0) == -9876);
	printf("> STRING \"-9876\" → INT -9876: ");
	fy_generic_emit_default(result);

	/* STRING → INT (invalid - should return fy_invalid) */
	v = fy_gb_string_create(gb, "not a number");
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> STRING \"not a number\" → INVALID (expected)\n");

	v = fy_gb_string_create(gb, "");
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> STRING \"\" → INVALID (expected)\n");

	/* FLOAT NaN/Infinity → INT (error) */
	v = fy_to_generic(0.0 / 0.0);  /* NaN */
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> FLOAT NaN → INVALID (expected)\n");

	/* NULL → INT (error) */
	v = fy_null;
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> NULL → INVALID (expected)\n");

	/* SEQUENCE → INT (error) */
	v = fy_sequence(1, 2, 3);
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> SEQUENCE → INVALID (expected)\n");

	/* MAPPING → INT (error) */
	v = fy_mapping("key", "value");
	result = fy_gb_to_int(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> MAPPING → INVALID (expected)\n");
...
START_TEST(conversion_to_float)
{
	char buf[16384];
	struct fy_generic_builder *gb;
	fy_generic v, result;
	...
	v = fy_to_generic(3.14159);
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	dv = fy_cast(result, 0.0);
	ck_assert(dv > 3.14158 && dv < 3.14160);
	printf("> FLOAT 3.14159 → FLOAT 3.14159: ");
	fy_generic_emit_default(result);
	...
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	ck_assert(fy_cast(result, 0.0) == 42.0);
	printf("> INT 42 → FLOAT 42.0: ");
	fy_generic_emit_default(result);
	...
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	ck_assert(fy_cast(result, -1.0) == 1.0);
	printf("> BOOL true → FLOAT 1.0: ");
	fy_generic_emit_default(result);

	v = fy_to_generic((_Bool)false);
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	ck_assert(fy_cast(result, -1.0) == 0.0);
	printf("> BOOL false → FLOAT 0.0: ");
	fy_generic_emit_default(result);

	/* STRING → FLOAT */
	v = fy_gb_string_create(gb, "2.71828");
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	dv = fy_cast(result, 0.0);
	ck_assert(dv > 2.71827 && dv < 2.71829);
	printf("> STRING \"2.71828\" → FLOAT 2.71828: ");
	fy_generic_emit_default(result);

	v = fy_gb_string_create(gb, "-123.456");
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_float(result));
	dv = fy_cast(result, 0.0);
	ck_assert(dv > -123.457 && dv < -123.455);
	printf("> STRING \"-123.456\" → FLOAT -123.456: ");
	fy_generic_emit_default(result);

	/* STRING → FLOAT (invalid) */
	v = fy_gb_string_create(gb, "not a float");
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> STRING \"not a float\" → INVALID (expected)\n");
	...
	v = fy_sequence(1.0, 2.0);
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> SEQUENCE → INVALID (expected)\n");

	/* MAPPING → FLOAT (error) */
	v = fy_mapping("pi", 3.14);
	result = fy_gb_to_float(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> MAPPING → INVALID (expected)\n");
...
/* Test: Type conversion to string (schema-aware) */
START_TEST(conversion_to_string)
{
	char buf[16384];
	struct fy_generic_builder *gb;
	fy_generic v, result;
	...
	v = fy_gb_string_create(gb, "hello");
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "hello");
	printf("> STRING \"hello\" → STRING \"hello\": ");
	fy_generic_emit_default(result);
	...
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "42");
	printf("> INT 42 → STRING \"42\": ");
	fy_generic_emit_default(result);

	v = fy_to_generic(-123);
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "-123");
	printf("> INT -123 → STRING \"-123\": ");
	fy_generic_emit_default(result);

	/* FLOAT → STRING */
	v = fy_to_generic(3.14);
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	str = fy_cast(result, "");
	printf("> FLOAT 3.14 → STRING \"%s\": ", str);
	fy_generic_emit_default(result);

	/* BOOL → STRING (YAML schema: lowercase) */
	v = fy_to_generic((_Bool)true);
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "true");
	printf("> BOOL true → STRING \"true\" (YAML): ");
	fy_generic_emit_default(result);

	v = fy_to_generic((_Bool)false);
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "false");
	printf("> BOOL false → STRING \"false\" (YAML): ");
	fy_generic_emit_default(result);

	/* NULL → STRING (YAML schema: "null") */
	v = fy_null;
	result = fy_gb_to_string(gb, v);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "null");
	printf("> NULL → STRING \"null\" (YAML): ");
	fy_generic_emit_default(result);

	/* Test with PYTHON schema - use separate buffer */
	{
		char buf2[16384];
		struct fy_generic_builder *gb2;

		gb2 = fy_generic_builder_create_in_place(FYGBCF_SCHEMA_PYTHON | FYGBCF_SCOPE_LEADER, NULL,
				buf2, sizeof(buf2));
		ck_assert_ptr_ne(gb2, NULL);

		enum fy_generic_schema schema = fy_gb_get_schema(gb2);
		ck_assert(schema == FYGS_PYTHON);

		/* BOOL → STRING (PYTHON schema: capitalized) */
		v = fy_to_generic((_Bool)true);
		result = fy_gb_to_string(gb2, v);
		ck_assert(fy_generic_is_string(result));
		str = fy_cast(result, "");
		ck_assert_str_eq(str, "True");
		printf("> BOOL true → STRING \"True\" (PYTHON): ");
		fy_generic_emit_default(result);
		...
		ck_assert(fy_generic_is_string(result));
		ck_assert_str_eq(fy_cast(result, ""), "False");
		printf("> BOOL false → STRING \"False\" (PYTHON): ");
		fy_generic_emit_default(result);

		/* NULL → STRING (PYTHON schema: "None") */
		v = fy_null;
		result = fy_gb_to_string(gb2, v);
		ck_assert(fy_generic_is_string(result));
		ck_assert_str_eq(fy_cast(result, ""), "None");
		printf("> NULL → STRING \"None\" (PYTHON): ");
		fy_generic_emit_default(result);

		/* SEQUENCE → STRING (flow format) */
		v = fy_sequence(1, 2, 3);
		result = fy_gb_to_string(gb2, v);
		ck_assert(fy_generic_is_string(result));
		str = fy_cast(result, "");
		printf("> SEQUENCE [1,2,3] → STRING \"%s\": ", str);
		fy_generic_emit_default(result);

		/* MAPPING → STRING (flow format) */
		v = fy_mapping("a", 1, "b", 2);
		result = fy_gb_to_string(gb2, v);
		ck_assert(fy_generic_is_string(result));
		str = fy_cast(result, "");
		printf("> MAPPING {a:1,b:2} → STRING \"%s\": ", str);
		fy_generic_emit_default(result);
	}

	printf("> All fy_gb_to_string() tests passed!\n");
}
END_TEST

/* Test: Type conversion to bool (schema-aware) */
START_TEST(conversion_to_bool)
{
	char buf[16384];
	struct fy_generic_builder *gb;
	fy_generic v, result;
	...
	/* YAML schema tests */
	gb = fy_generic_builder_create_in_place(FYGBCF_SCHEMA_YAML1_2_CORE | FYGBCF_SCOPE_LEADER, NULL,
			buf, sizeof(buf));
	ck_assert_ptr_ne(gb, NULL);

	/* BOOL → BOOL (identity) */
	v = fy_to_generic((_Bool)true);
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_bool(result));
	ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
	printf("> BOOL true → BOOL true: ");
	fy_generic_emit_default(result);

	v = fy_to_generic((_Bool)false);
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_bool(result));
	ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
	printf("> BOOL false → BOOL false: ");
	fy_generic_emit_default(result);
	...
	ck_assert(fy_generic_is_bool(result));
	ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
	printf("> FLOAT 3.14 → BOOL true: ");
	fy_generic_emit_default(result);

	/* STRING → BOOL (YAML 1.2 Core parsing) */
	v = fy_gb_string_create(gb, "true");
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_bool(result));
	ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
	printf("> STRING \"true\" → BOOL true (YAML): ");
	fy_generic_emit_default(result);

	v = fy_gb_string_create(gb, "False");
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_bool(result));
	ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
	printf("> STRING \"False\" → BOOL false (YAML): ");
	fy_generic_emit_default(result);
	...
	/* STRING → BOOL (invalid for YAML) */
	v = fy_gb_string_create(gb, "not a bool");
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> STRING \"not a bool\" → INVALID (YAML, expected)\n");

	/* NULL → BOOL (error for YAML) */
	v = fy_null;
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> NULL → INVALID (YAML, expected)\n");

	/* SEQUENCE → BOOL (error for YAML) */
	v = fy_sequence(1, 2);
	result = fy_gb_to_bool(gb, v);
	ck_assert(fy_generic_is_invalid(result));
	printf("> SEQUENCE → INVALID (YAML, expected)\n");

	/* Test with PYTHON schema (truthiness) - use separate buffer */
	{
		char buf2[16384];
		struct fy_generic_builder *gb2;
		...
		/* STRING → BOOL (Python: empty → False, non-empty → True) */
		v = fy_gb_string_create(gb2, "");
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
		printf("> STRING \"\" → BOOL false (PYTHON): ");
		fy_generic_emit_default(result);

		v = fy_gb_string_create(gb2, "hello");
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
		printf("> STRING \"hello\" → BOOL true (PYTHON): ");
		fy_generic_emit_default(result);

		/* Note: even "False" as a string is True in Python! */
		v = fy_gb_string_create(gb2, "False");
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
		printf("> STRING \"False\" → BOOL true (PYTHON truthiness): ");
		fy_generic_emit_default(result);

		/* NULL → BOOL (Python: False) */
		v = fy_null;
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
		printf("> NULL → BOOL false (PYTHON): ");
		fy_generic_emit_default(result);

		/* SEQUENCE → BOOL (Python: empty → False, non-empty → True) */
		v = fy_sequence();
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
		printf("> SEQUENCE [] → BOOL false (PYTHON): ");
		fy_generic_emit_default(result);
		...
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
		printf("> SEQUENCE [1,2,3] → BOOL true (PYTHON): ");
		fy_generic_emit_default(result);

		/* MAPPING → BOOL (Python: empty → False, non-empty → True) */
		v = fy_mapping();
		result = fy_gb_to_bool(gb2, v);
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)true) == (_Bool)false);
		printf("> MAPPING {} → BOOL false (PYTHON): ");
		fy_generic_emit_default(result);
		...
		ck_assert(fy_generic_is_bool(result));
		ck_assert(fy_cast(result, (_Bool)false) == (_Bool)true);
		printf("> MAPPING {key:value} → BOOL true (PYTHON): ");
		fy_generic_emit_default(result);
	}

	printf("> All fy_gb_to_bool() tests passed!\n");
}
END_TEST

/* Test: convert op */
START_TEST(convert_op)
{
	fy_generic v, result;
	const char *str;
	...
	/* INT → STRING */
	v = fy_to_generic(42);
	result = fy_convert(v, FYGT_STRING);
	ck_assert(fy_generic_is_string(result));
	ck_assert_str_eq(fy_cast(result, ""), "42");
	printf("> convert 42 → STRING \"42\": ");
	fy_generic_emit_default(result);
	...
	ck_assert(fy_generic_is_string(result));
	str = fy_cast(result, "");
	printf("> convert SEQUENCE [1,2,3] → STRING \"%s\": ", str);
	fy_generic_emit_default(result);

	printf("> All convert op tests passed!\n");
}
END_TEST
...
START_TEST(generic_iterator)
{
	...
	ck_assert(fy_generic_is_valid(vdir));
	fy_generic_emit_default(vdir);

	vroot = fy_get(vdir, "root", fy_invalid);
	ck_assert(fy_generic_is_valid(vroot));
	fy_generic_emit_default(vroot);

	/* Create iterator */
	fygi = fy_generic_iterator_create();
	ck_assert_ptr_ne(fygi, NULL);

	/* Start iteration from root */
	fy_generic_iterator_generic_start(fygi, vroot);

	/* Count all nodes */
	count = 0;
	while (fy_generic_is_valid(v = fy_generic_iterator_generic_next(fygi))) {
		printf("[%d]: ", count);
		fy_generic_emit_default(v);
...
/* Test: Generic iterator functionality */
START_TEST(generic_iterator_events)
{
	char buf[16384];
	struct fy_generic_builder *gb;
	...
	while ((fye = fy_generic_iterator_generate_next(fygi)) != NULL) {
		evtxt = fy_event_to_string(fye);
		ck_assert_ptr_ne(evtxt, NULL);
...
void libfyaml_case_generic(struct fy_check_suite *cs)
{
	...
	ctc = fy_check_suite_add_test_case(cs, "generic");
	...
	fy_check_testcase_add_test(ctc, generic_size_encoding);
	fy_check_testcase_add_test(ctc, generic_string_range);
	fy_check_testcase_add_test(ctc, generic_type_promotion);

	/* invalid propagation tests */
	fy_check_testcase_add_test(ctc, generic_invalid_propagation);

	/* sized string (any kind of data including zeroes) */
	fy_check_testcase_add_test(ctc, generic_sized_string);

	/* decorated int */
	fy_check_testcase_add_test(ctc, generic_decorated_int);

	/* casts */
	fy_check_testcase_add_test(ctc, generic_casts);

	/* get */
	fy_check_testcase_add_test(ctc, generic_get);

	/* get_at testing */
	fy_check_testcase_add_test(ctc, generic_get_at);

	/* compare */
	fy_check_testcase_add_test(ctc, generic_compare);
	...
	fy_check_testcase_add_test(ctc, gb_dedup_scoping2);

	/* the polymorphic stuff */
	fy_check_testcase_add_test(ctc, gb_polymorphics);

	/* time to do operations on collection */
	fy_check_testcase_add_test(ctc, gb_basic_collection_ops);
	fy_check_testcase_add_test(ctc, gb_assoc_deassoc);

	/* map utils */
	fy_check_testcase_add_test(ctc, gb_map_utils);

	/* seq utils */
	fy_check_testcase_add_test(ctc, gb_seq_utils);

	/* filter/map/reduce */
	fy_check_testcase_add_test(ctc, gb_filter);
	fy_check_testcase_add_test(ctc, gb_map);
	fy_check_testcase_add_test(ctc, gb_reduce);

	/* parallel filter/map/reduce */
	fy_check_testcase_add_test(ctc, gb_pfilter);
	fy_check_testcase_add_test(ctc, gb_pmap);
	fy_check_testcase_add_test(ctc, gb_preduce);

	/* now time to do collection operations using the simple intefaces */
	fy_check_testcase_add_test(ctc, simple_ops);

	/* iterator test */
	fy_check_testcase_add_test(ctc, iterators);

	/* local operations */
	fy_check_testcase_add_test(ctc, local_ops);
...
#ifdef FY_HAVE_LAMBDAS
	/* lambda operations */
	fy_check_testcase_add_test(ctc, lambda_ops);
#endif

	/* get_at_path macros */
	fy_check_testcase_add_test(ctc, get_at_path);
	...
	fy_check_testcase_add_test(ctc, take_drop_ops);

	/* polymorphic wrapper macros for slice operations */
	fy_check_testcase_add_test(ctc, polymorphic_slice_ops);

	/* type conversion operations */
	fy_check_testcase_add_test(ctc, conversion_to_int);
	fy_check_testcase_add_test(ctc, conversion_to_float);
