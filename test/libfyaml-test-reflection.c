...
/* Test: malformed required entry meta must fail cleanly without leaking allocations */
START_TEST(reflection_invalid_required_entry_meta_fails_cleanly)
{
	static const char invalid_meta[] = "! {required}";
	struct fy_reflection *rfl;
	struct fy_type_context_cfg ctx_cfg = { 0 };
	struct fy_type_context *ctx;
	...
	ctx_cfg.rfl = rfl;
	ctx_cfg.entry_type = "unsigned char";
	ctx_cfg.entry_meta = invalid_meta;
	ctx = fy_type_context_create(&ctx_cfg);
	ck_assert_ptr_eq(ctx, NULL);
